...
const AdminServiceEdit = () => {
    ...
    useEffect(() => {
        ...
        axios.get(`/api/service/view-by-id/${city_id}`, config)
            .then((response) => {
                ...
                setCity(response.data.result[0].city_id)
                setUrl(response.data.result[0].url)
                setLogoDemo(`http://127.0.0.1:8000/service/logo/${response.data.result[0].logo}`)
                setCityDemo(response.data.result[0].city_name)
                dispatch(hide())
            })
            .catch((error) => {
                console.log(error)
    ...
    const serviceFormHandler = (e) => {
        ...
        if (logo === null) {
            if (title.length === 0 || url.length === 0 || city.length === 0) {
                setError(true)
